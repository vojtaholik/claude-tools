---
name: ralph-prd
description: Start a PRD-driven ralph loop. Use when the user says /ralph-prd or wants autonomous story-by-story development from a prd.json config.
argument-hint: "[init]"
---
...
```bash
~/.claude/plugins/local/claude-tools/ralph-prd/scripts/prd-init.sh "<project>" '<stories_json>'
```
...
```bash
~/.claude/plugins/local/claude-tools/ralph-prd/scripts/prd-next.sh
```
